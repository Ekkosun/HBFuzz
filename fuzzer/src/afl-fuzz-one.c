...
int fuzz_one_hbf(afl_state_t *afl);
/* MOpt */
...
static void sort_list(struct range **head) {
    if (*head == NULL) return;

    int swapped;
    struct range *ptr1;
    struct range *lptr = NULL;
...
static void merge_intervals(struct range **head) {
    if (*head == NULL) return;

    struct range *current = *head;
    struct range *next_range;

    while (current->next != NULL) {
        next_range = current->next;
        if (current->end >= next_range->start) { // Check for overlap
            if (current->end < next_range->end) {
                current->end = next_range->end;
            }
            // Remove next_range
            current->next = next_range->next;
            if (next_range->next != NULL) {
                next_range->next->prev = current;
            }
            free(next_range);
        } else {
...
static void dump_list(struct range * head){
  struct range * rng = head;
  fprintf(stderr, "start to dump current list\n");
  while(rng){
    fprintf(stderr, "(%d %d)\n", rng->start, rng->end);
    rng = rng->next;
  }
...
// static void remove_duplicates(struct range **head) {
//     if (*head == NULL) return;
...
//             if (current->start == next_range->start && current->end == next_range->end) {
//                 // Remove next_range
//                 if (next_range->next != NULL) {
//                     next_range->next->prev = current;
//                 }
...
inline void discover_word_no_change(u8 *ret, u32 *current, u32 *virgin) {
...
inline u8 has_new_bits_no_change(afl_state_t *afl, u8 *virgin_map) {
  ...
  while (i--) {
    ...
    if (unlikely(*current)) discover_word_no_change(&ret, current, virgin);
...
void havoc_mutation(u8* data, u64 len, u32 steps) {
    if (len == 0) return;
...
int fuzz_one_hbf(afl_state_t *afl){
  u64 before_hbf_time = get_cur_time();
  char * patched_cmd = getenv("AFL_HBF_CMD");
  char * tmp_file = getenv("AFL_HBF_TMP_FILE");
  struct range   *ranges = NULL, *cricial_ranges = NULL, *rng;
  ...
  if (fp == NULL) {
    fprintf(stdout, "\nfailed to open tmp file in hbf\n");
    goto hbf_failed;
  }
  fwrite(buf, 1, len, fp);
  fclose(fp);
  FILE * s_fp = popen(patched_cmd, "r");
  while (fgets(stdout_buf, sizeof(stdout_buf), s_fp)){
    if (strncmp(stdout_buf, "#####", 5) == 0) {
      long size, count, pos;
      if (sscanf(stdout_buf, "##### %*p %ld %ld %ld %*p", &size, &count, &pos) == 3) {
        // Allocate/reallocate memory for storing values
  ...
  pclose(s_fp);
  if(ranges == NULL) {goto hbf_failed;}

  // rng=ranges;
  // while(rng){
  //   fprintf(stderr,"Pre Range: [%u, %u]\n", rng->start, rng->end);
  //   rng = rng->next;
  // }
  ...
  // rng=ranges;
  // while(rng){
  //   fprintf(stderr,"After Range: [%u, %u]\n", rng->start, rng->end);
  //   rng = rng->next;
  // }
  u32 stack_max = 1 << (1 + rand_below(afl, afl->havoc_stack_pow2));
  memcpy(backup, buf, len);
  afl->stage_name = "hbf-havoc-internal";
  afl->stage_short = "hbfhi";
  afl->stage_max = (HAVOC_CYCLES * perf_score / afl->havoc_div) >> 8;
  afl->stage_cur = 0;
  u64 cur_time = get_cur_time();
  u64 timeout = 20000;
  show_stats(afl);
  u64 hbf_havoc_queued = afl->queued_items;
  while(afl->stage_cur < afl->stage_max){
    rng = ranges;
    while(afl->stage_cur < afl->stage_max && rng){
      int start = rng->start;
      int end = rng->end;
      memcpy(changed, buf + start, end - start + 1);
      // afl_mutate(afl, changed, end - start + 1, 1 + rand_below(afl, stack_max), false, true, NULL, 0,  (end - start + 1) * 2);
      havoc_mutation(changed, end - start + 1, 256);
      memcpy(buf + start, changed, end - start + 1);
    ...
    if (afl->queued_items != hbf_havoc_queued && afl->stage_max < 2048 * 5) {
      afl->stage_max *= 2;
      timeout += (afl->queued_items - hbf_havoc_queued) * 1000;
      hbf_havoc_queued = afl->queued_items;
    }
    if (get_cur_time() - cur_time > timeout) {break;}
  }

  afl->stage_name = "hbf-havoc-extern";
  afl->stage_short = "hbfhe";
  afl->stage_max = (HAVOC_CYCLES * perf_score / afl->havoc_div) >>8;
  afl->stage_cur = 0;
  ...
  while(afl->stage_cur < afl->stage_max){
    ...
    rng = ranges;
    while(rng){
      if (rand_below(afl, 100) < 60) {continue;}
      int start = rng->start;
      int end = rng->end;
      memcpy(changed, buf + start, end - start + 1);
      // afl_mutate(afl, changed, end - start + 1, 1 + rand_below(afl, stack_max), false, true, NULL, 0, (end - start + 1) * 2);
  ...
  while (cricial_ranges) {

    rng = cricial_ranges;
    cricial_ranges = rng->next;
    ck_free(rng);
    rng = NULL;
